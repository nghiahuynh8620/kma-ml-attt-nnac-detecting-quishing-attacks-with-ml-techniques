...
- **Dự đoán QR** từ 2 nguồn:
  ...
  - **Text nhập tay**: nhập nội dung QR thủ công để kiểm thử nhanh.
- **Benchmark hiệu năng**:
  - đo thời gian sinh QR,
  - tiền xử lý,
  - suy luận mô hình,
  - độ trễ end-to-end,
  - CPU / RAM khi chạy demo.
- **Ghi log** để phục vụ báo cáo và so sánh hiệu năng giữa các lần chạy.

### Luồng hoạt động
1. Người dùng chọn model đã train.
2. Chọn nguồn QR: synthetic hoặc nhập tay.
3. Hệ thống render nội dung thành ảnh QR.
4. Ảnh QR được resize về kích thước đầu vào mô hình.
5. Mô hình dự đoán nhãn và trả về điểm số lớp positive.
6. Kết quả và log hiệu năng được hiển thị trên dashboard.

### Cấu trúc thư mục liên quan
```text
outputs/
├── models/              # các model .joblib đã train
├── webapp_logs/         # log dự đoán và benchmark
│   ├── webapp_perf_log.csv
│   └── webapp_benchmark_log.csv
```

### Lưu ý sử dụng
- Các mẫu **malicious** trong app chỉ là **payload mô phỏng an toàn** để thử nghiệm mô hình.
- Kết quả trong webapp là **demo minh họa ứng dụng**, không thay thế cho đánh giá thực nghiệm chính thức trong notebook / báo cáo.
- Khi báo cáo hiệu năng, nên dùng thêm các chỉ số như **mean**, **std**, **p95**, **p99**, **max latency**.

### Cách chạy
```bash
streamlit run app_qr_demo_streamlit_v2_intro.py
```
"""
...
with intro_tab:
    ...
    i2.success('**Đầu ra**\n\nNhãn dự đoán, điểm số lớp 1 và thời gian xử lý.')
    i3.warning('**Mục đích**\n\nMinh họa ứng dụng và hỗ trợ benchmark webapp.')

    st.subheader('Thông tin phiên chạy hiện tại')
    st.write({
        'selected_model': model_path,
